...
std::shared_ptr<Level> LevelReader::build_level(int level_num) {
  ...
  enemy_manager->set_staritng_wave(BaseEnemy::EnemyType::ANT,
                                   ant_starting_wave);
...
std::string LevelReader::get_level_path(int level_num = -1) {
  if(level_num == -1) return std::filesystem::current_path().string() + levels_path;

  return std::filesystem::current_path().string() + levels_path + std::to_string(level_num)+".txt";
}