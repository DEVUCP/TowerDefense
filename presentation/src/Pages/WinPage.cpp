...
WinPage::WinPage(unsigned width, unsigned height)
  ...
  score_text = std::make_shared<Text>("Score: " + std::to_string(Game::get_instance().get_level()->get_score()), 80, sf::Color::White, start_x, start_y + score_text_offset_y);

  restart_button =
      std::make_shared<IconButton>("./assets/buttons/Restart.png",
                                   "./assets/buttons/Restart_Hover.png", start_x + buttons_spacing_x*button_offset_multiplier++, start_y + buttons_offset_y);
  ...
  continue_button =
    std::make_shared<IconButton>("./assets/buttons/Play.png",
                                 "./assets/buttons/Play_Hover.png", start_x + buttons_spacing_x*button_offset_multiplier++, start_y + buttons_offset_y);
  continue_button->set_handler([this]() {
    if(Game::get_instance().get_level()->get_level_num() < Game::get_instance().get_level()->get_level_count()) {
      Game::get_instance().init_level(Game::get_instance().get_level()->get_level_num() + 1);
      notify_observers(Event::GAME_PAGE_SWITCH);
    } else {
      notify_observers(Event::MENU_MENU_SWITCH);
    }
  });

  levels_button =
      std::make_shared<IconButton>("./assets/buttons/Levels.png",
                                   "./assets/buttons/Levels_Hover.png", start_x + buttons_spacing_x*button_offset_multiplier++, start_y + buttons_offset_y);
